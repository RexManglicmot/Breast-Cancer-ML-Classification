...
A special acknowledgement to the University of Irvine Data Repository for providing the dataset to the public.^[https://archive.ics.uci.edu/ml/datasets/gene+expression+cancer+RNA-Seq] A special acknowledgement for Samuele Fiorini from University of Genoa for the original dataset.^[Samuele Fiorini, samuele.fiorini '@' dibris.unige.it, University of Genoa]

The dataset contains 20,531 genes and unfortunately the name of the genes could not be identified (still in the processing of contacting dataset owner). There are 801 observations and each observation is classified as having one of 5 types of cancers.
...
#dim(data)
```
the data dataset with >20K columns abd the data_label has the classification of the patient.

## Cleaning the Data
First, we need to do is combine the datasets by using an join function by using the X variable and store the new data set into an new object, data2. 
```{r}
...
```
We see that data we have a total of 20532 and in data2 we 20533, we can assume that we added the Class column. But just to make sure, we called the which function that allowed to us locate the Class column, which was 30532. We have successfully added the column.

Now for more data cleaning, we have quite a few genes, how about we see and get rid of genes that are not present in any of the 801 observations.
